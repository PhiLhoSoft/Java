 ...
 * Some improvements (mostly for Tiff) by Philippe Lhoste
 *
 ...
 *  <li><strong>2009/10/28</strong>Philippe Lhoste: Added code for Tiff images.
 *   Released as (no increment).</li>
 * </ul>
...
public class ImageInfo {
   ...
   public static final int COLOR_TYPE_TRUECOLOR_RGB = 0; // 24 bits per pixel
   public static final int COLOR_TYPE_PALETTED = 1; // 8 bits per pixel but 24 bits per color
   public static final int COLOR_TYPE_GRAYSCALE = 2; // 8 bits per pixel
   public static final int COLOR_TYPE_BILEVEL = 3; // Often black & white, 1 bit per pixel
   public static final int COLOR_TYPE_CMYK = 4;

   private static final String[] COLOR_TYPES =
      {"Truecolor RGB", "Paletted", "Grayscale", "Bilevel", "CMYK"};
   ...
   public boolean check() {
      ...
      comments = null;
      try {
         // Do a first check on the first two bytes of the file,
         // to route to the right checker
         int b1 = read() & 0xff;
         int b2 = read() & 0xff;
         if (b1 == 0x47 && b2 == 0x49) {	// "GI"
            return checkGif();
         }
         else
         if (b1 == 0x89 && b2 == 0x50) {	// "‰P"
            return checkPng();
         }
         else
         if (b1 == 0xff && b2 == 0xd8) {	// "BØ"
            return checkJpeg();
         }
         else
         if (b1 == 0x42 && b2 == 0x4d) {
         ...
         if (b1 == 0x46 && b2 == 0x4f) {	// "FO"
            return checkIff();
         }
         else
         if (b1 == 0x59 && b2 == 0xa6) {	// "Y¦"
            return checkRas();
         }
         else
         if (b1 == 0x50 && b2 >= 0x31 && b2 <= 0x36) { // "P1" to "P6"
            return checkPnm(b2 - '0');
         }
         else
         if (b1 == 0x38 && b2 == 0x42) {	// "8B"
            return checkPsd();
         }
         else
         if (b1 == 0x49 && b2 == 0x49) { // "II"
            return checkTiff(true);	// Little endian
         }
         else
         if (b1 == 0x4D && b2 == 0x4D) { // "MM"
            return checkTiff(false);	// Big endian
         }
         else {
            return false;
         }
   ...
   private boolean checkGif() throws IOException {
      final byte[] GIF_MAGIC_87A = {0x46, 0x38, 0x37, 0x61}; // "F87a"
      final byte[] GIF_MAGIC_89A = {0x46, 0x38, 0x39, 0x61}; // "F89a"
   ...
   private boolean checkJpeg() throws IOException {
      ...
      while (true) {
         ...
         if (marker >= 0xffc0 && marker <= 0xffcf && marker != 0xffc4 && marker != 0xffc8) {
            ...
            format = FORMAT_JPEG;
            colorType = COLOR_TYPE_TRUECOLOR_RGB;
   ...
   private boolean checkPnm(int id) throws IOException {
      ...
      // See http://ozviz.wasp.uwa.edu.au/~pbourke/dataformats/ppm/
      final int[] PNM_FORMATS = {FORMAT_PBM, FORMAT_PGM, FORMAT_PPM};
      format = PNM_FORMATS[(id - 1) % 3];
      switch (id) {
         case 1: colorType = COLOR_TYPE_BILEVEL; break; // PBM
         case 2: colorType = COLOR_TYPE_GRAYSCALE; break; // PGM (Ascii format)
         case 3: colorType = COLOR_TYPE_TRUECOLOR_RGB; break; // PPM (Ascii format)
         case 4: colorType = COLOR_TYPE_BILEVEL; break; // PBM (binary format) (exists?)
         case 5: colorType = COLOR_TYPE_GRAYSCALE; break; // PGM (binary format)
         case 6: colorType = COLOR_TYPE_TRUECOLOR_RGB; break; // PPM (binary format)
...
private boolean checkTiff(boolean bUseLittleEndian) throws IOException {
      ...
      final byte[] TIFF_MAGIC_BE = {0x00, 0x2A};
      final byte[] TIFF_MAGIC_LE = {0x2A, 0x00};
      if (!equals(a, 0, bUseLittleEndian ? TIFF_MAGIC_LE : TIFF_MAGIC_BE, 0, 2)) {
         return false;
      }
      format = FORMAT_TIFF;

      // Must use a seekable stream
      if (din == null || !(din instanceof FileImageInputStream))
         return checkTiffHeader(bUseLittleEndian);
      FileImageInputStream fiis = (FileImageInputStream) din;

      if (bUseLittleEndian) {
         fiis.setByteOrder(ByteOrder.LITTLE_ENDIAN);
      } else {
         fiis.setByteOrder(ByteOrder.BIG_ENDIAN);
      }

      // Read the IFD (image file directory) offset
      long offset = fiis.readUnsignedInt();
      fiis.seek(offset);
      // We are now in the IFD, which starts with the number of tags/fields/directory entries


      // Some intermediary variables to cumulate values before consolidating the whole
      short resolutionUnit = 2;	// Default
      long xr = -1, yr = -1;	// Resolutions (offset, actually)
      short photometricInterpretation = -1; // 0 or 1 = bilevel or grayscale, 2 = RGB full color, 3 = palette
      short samplesPerPixel = 1; // 1 (not defined, actually) for bilevel, gray and palette, 3 for RGB, 4 or more for CMYK (or RGBA)
      long bitsPerSampleOffset = -1; // If several samples per pixel
      short bitsPerSample = 1;	// Default if not defined (bilevel images)
      short compression = -1;
      long commentOffset = -1;
      int commentLen = -1;

      int tagNb = fiis.readUnsignedShort();
      for (int i = 0; i < tagNb; i++) {
         int tagType = fiis.readUnsignedShort();
         int fieldType = fiis.readUnsignedShort(); // 1=BYTE, 2=ASCII, 3=SHORT, 4=LONG, 5=RATIONAL, other...
         long count = fiis.readUnsignedInt(); // Number of values
         long value = -1, valueOffset = -1;
         if (fieldType == 3) { // SHORT, 2 bytes, left aligned in the field
            value = fiis.readUnsignedShort();
            fiis.readUnsignedShort(); // Drop remainder
         } else { // LONG (8 btytes) or offset
            value = valueOffset = fiis.readUnsignedInt();
         }
         switch (tagType) {
            case 256: // 0x100: ImageWidth (SHORT or LONG)
               width = (int) value;
               break;
            case 257: // 0x101: ImageLength (SHORT or LONG)
               height = (int) value;
               break;
            case 258: // 0x102: BitsPerSample (nb of bits per component)
               bitsPerSampleOffset = valueOffset;
               bitsPerSample = (short) value;
               break;
            case 259: // 0x103: Compression
               compression = (short) value;
               break;
            case 262: // 0x106: PhotometricInterpretation
               photometricInterpretation = (short) value;
               break;
            case 270: // 0x10E: Comment
               commentOffset = valueOffset;
               commentLen = (int) count;
               break;
            case 277: // 0x115: SamplesPerPixel (nb of components per pixel)
               samplesPerPixel = (short) value;
               break;
            case 282: // 0x11A: XResolution (rational)
               xr = valueOffset;
               break;
            case 283: // 0x11B: YResolution (rational)
               yr = valueOffset;
               break;
            case 296: // 0x128: ResolutionUnit
               resolutionUnit = (short) value; // 1=none, 2=Inch, 3=Centimeter
               break;
         }
      }
      numberOfImages = 1; // Tiff files can have several images but for today, we look only at the first one...

      if (samplesPerPixel > 1) {
         // The BitsPerSample value is actually an offset to several values (one per component/sample)
         if (bitsPerSampleOffset == -1) {
            // bitsPerSample can be actually a SHORT inside the IFD,
            // defining the total number of bits per pixels,
            // which is against the spec, but I have meet this case
            // (with classical reference Lena image, 12 bits, none the less! Old version of spec?)
            bitsPerPixel = bitsPerSample;
         } else {
            fiis.seek(bitsPerSampleOffset);
            bitsPerPixel = 0;
            for (int i = 0; i < samplesPerPixel; i++) {
               bitsPerPixel += fiis.readUnsignedShort();
            }
         }
      } else {	// 1 sample per pixel (bilevel, graylevel or palette image)
         bitsPerPixel = bitsPerSample;
      }

      if (xr != -1) {
         // Fetch resolution value
         fiis.seek(xr);
         float xrf = GetRational(fiis);
         if (resolutionUnit == 1 || resolutionUnit == 2) { // No unit or inch
            setPhysicalWidthDpi((int) xrf);
         }
         else if (resolutionUnit == 3) { // Centimeter
            setPhysicalWidthDpi((int) (xrf * 2.54f));
         }
      }
      if (yr != -1) {
         // Fetch resolution value
         fiis.seek(yr);
         float yrf = GetRational(fiis);
         if (resolutionUnit == 1 || resolutionUnit == 2) {
            setPhysicalHeightDpi((int) yrf);
         } else if (resolutionUnit == 3) {
            setPhysicalHeightDpi((int) (yrf * 2.54f));
         }
      }

      if (collectComments && commentOffset > 0) {
         // Comment
         fiis.seek(commentOffset);
         byte[] chars = new byte[commentLen];
         if (read(chars, 0, commentLen) != commentLen) {
            return false;
         }
         String comment = new String(chars, "iso-8859-1");
         comment = comment.trim();
      ...
      GuessTiffColorType(photometricInterpretation);
...
private boolean checkTiffHeader(boolean bUseLittleEndian) throws IOException {
      ...
      int offset = (int) er.getInt(a, 0);
      skip(offset - 8); // We have read 8 bytes so far
      // We are now in the IFD, which starts with the number of tags/fields/directory entries
      ...
      int tagNb = er.getShort(a, 0);
      for (int i = 0; i < tagNb; i++) {
         byte[] ifbEntry = new byte[12];
         if (read(ifbEntry) != ifbEntry.length) {
            return false;
         }
         int tagType = er.getShort(ifbEntry, 0);
         int fieldType = er.getShort(ifbEntry, 2); // 1=BYTE, 2=ASCII, 3=SHORT, 4=LONG, 5=RATIONAL, other...
         long count = er.getInt(ifbEntry, 4); // Number of values
         long value = -1, valueOffset = -1;
         if (fieldType == 3) { // SHORT, 2 bytes, left aligned in the field
            value = er.getShort(ifbEntry, 8);
         } else { // LONG (8 btytes) or offset
            value = valueOffset = er.getInt(ifbEntry, 8);
         }
         switch (tagType) {
            case 256: // 0x100: ImageWidth (SHORT or LONG)
               width = (int) value;
      ...
      if (samplesPerPixel > 1) {
         ...
         if (bitsPerSampleOffset == -1) {
            ...
            // We just cannot get the information, needing a seekable offset, but we will make a reasonable guess...
            bitsPerPixel = 8 * samplesPerPixel; // Likely to be 24 for RGB
         }
      } else {	// 1 sample per pixel (bilevel, graylevel or palette image)
         bitsPerPixel = bitsPerSample;
      }
   ...
   class EndianReader {
      boolean bUseLittleEndian;
      public EndianReader(boolean bIsLittleEndian) {
   ...
   private float GetRational(FileImageInputStream fiis) throws IOException {
      long numerator = fiis.readUnsignedInt();
      long denominator = fiis.readUnsignedInt();
      return (float) numerator / denominator;
   }

   private void GuessTiffColorType(short photometricInterpretation) {
      // photometricInterpretation -- 0 or 1 = bilevel or grayscale, 2 = RGB full color, 3 = palette
      // samplesPerPixel -- 1 for bilevel, gray and palette, 3 for RGB, 4 or more for CMYK (or RGBA)
   ...
   private static boolean useTiffHeaderOnly(String[] args) {
      if (args != null && args.length > 0) {
         for (int i = 0; i < args.length; i++) {
            if ("-th".equals(args[i])) {
               return true;
   ...
   /**
    * If {@link #check()} was successful, returns the image color type as one
    * of the COLOR_TYPE_xyz constants from this class.
    * @return color type as a COLOR_TYPE_xyz constant
    */
   ...
   /**
    * If {@link #check()} was successful, returns the color type's name.
    * Use {@link #getColorType()} to get a unique number.
   ...
   public static void main(String[] args) {
      ...
      if (args.length == 0) {
         run(null, System.in, imageInfo, true);
      } else {
         boolean verbose = determineVerbosity(args);
         boolean useTiffHeader = useTiffHeaderOnly(args);
         int index = 0;
         while (index < args.length) {
            ...
            boolean httpStream = name.startsWith("http://");
            if (httpStream) {
               useTiffHeader = true; // Cannot use FileImageInputStream on such stream
            }

            if (!useTiffHeader && (name.toLowerCase().endsWith(".tif") || name.toLowerCase().endsWith(".tiff"))) {
               FileImageInputStream fiis = null;
               try {
                  // Don't try to support http, it isn't really a Web format anyway...
                  fiis = new FileImageInputStream(new File(name));
                  run(name, fiis, imageInfo, verbose);
               } catch (IOException e) {
   ...
   private static void printCompact(String sourceName, ImageInfo imageInfo) {
      ...
      System.out.println(
         ...
         imageInfo.getColorTypeName() + SEP +
   ...
   private static void printVerbose(String sourceName, ImageInfo ii) {
      ...
      printLine(1, "Color type: ", ii.getColorTypeName());
   ...
   private static void run(String sourceName, FileImageInputStream in, ImageInfo imageInfo, boolean verbose) {
      imageInfo.setInput(in);
      imageInfo.setDetermineImageNumber(true);
      imageInfo.setCollectComments(verbose);
      if (imageInfo.check()) {
