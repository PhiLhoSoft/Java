...
public class FileChooserTest2
{
	...
	private void openMulti()
	{
		...
		if (status == JFileChooser.APPROVE_OPTION)
		{
			...
			{
				System.out.println("Selected: " + aSelectedFiles[i].getParent() + " --- " + aSelectedFiles[i].getName());
			}
		}
	}
	...
	private void openMultiFilter()
	{
		...
		fileChooser.addChoosableFileFilter(filter);
		filter = new SimpleFileFilter(new String[] { "java", "class" }, "All Java Files");
		fileChooser.addChoosableFileFilter(filter);
...
class ExtendedJFileChooser extends JFileChooser
{
	...
	private void Init()
	{
		...
		if (fcUI instanceof BasicFileChooserUI)
		{
			...
			ListModel<Object> model = ((BasicFileChooserUI) fcUI).getModel();
	...
	/**
	 * Changing cursor for a component. Cursor will be changed at application level (not only for the component).
	 */
	...
	/**
	 * Bug workaround for Java Web Start 1.0 Under Windows, use the special WindowsAltFileSystemView which does not
	 * display the box "no disk"
	 */
	...
	public void setCurrentDirectory(File aPathFile)
	{
		...
		JComboBox<File> aCmb = (JComboBox<File>) GetCombo(this, 0);
		int nIdx = -1;
		...
		if (aCmb != null && aFile != null)
		{
			...
			DefaultComboBoxModel<File> cbm = new DefaultComboBoxModel<>();
...
class WindowsAltFileSystemView extends FileSystemView
{
	...
	/**
	 * Returns whether a file is hidden or not. On Windows there is currently no way to get this information from
	...
	/**
	 * Returns all root partitions on this system. On Windows, this will be the A: through Z: drives.
	 */
	@Override
