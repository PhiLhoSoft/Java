...
public class TestID3
{
			...
			throws UnsupportedEncodingException, IOException
	{
		...
		while (!foundImage)
		{
			...
			while (currentPosition < dataLength)
			{
				...
				byte[] frameBuffer = new byte[version2 ? 6 : 10];
				...
				String frameId = new String(frameBuffer, 0, version2 ? 3 : 4);
				destinationArray = new byte[version2 ? 3 : 4];
						...
						switch (encodingType)
						{
							...
							case 2:
								...
								do
								{
									...
									byte b22 = frameBuffer[scanPoint++];
									if (b1 == 0 && b2 == 0)
										break;
	...
	private int read32bit(byte[] data)
	{
		return
				((data[0] & 0xFF) << 24) |
				((data[1] & 0xFF) << 16) |
	...
	private int read28bit(byte[] data)
	{
		return
				((data[0] & 0xFF) << 21) |
				((data[1] & 0xFF) << 14) |
	...
	private int read24bit(byte[] data)
	{
		return
				((data[1] & 0xFF) << 16) |
				((data[2] & 0xFF) <<  8) |
