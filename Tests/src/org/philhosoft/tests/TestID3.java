...
public class TestID3
{
	...
	public static BufferedImage getID3Image(InputStream dataStream)
			throws UnsupportedEncodingException, IOException
	{
		...
		if (dataStream.read(buffer, 0, 10) != 10 || !(new String(buffer, 0, 3).equals("ID3")))
		{
			...
			if (DEBUG) System.out.println("Not ID3");
			return null;
		}
		currentPosition += 10;
		...
		while (currentPosition < dataLength)
		{
			...
			if (DEBUG) System.out.println("Found frame " + frameId);
	...
	private static final int read32bit(byte[] data)
	{
		return
				((data[0] & 0xFF) << 24) |
				((data[1] & 0xFF) << 16) |
	...
	private static final int read28bit(byte[] data)
	{
		return
				((data[0] & 0xFF) << 21) |
				((data[1] & 0xFF) << 14) |
	...
	private static final int read24bit(byte[] data)
	{
		return
				((data[1] & 0xFF) << 16) |
				((data[2] & 0xFF) <<  8) |
